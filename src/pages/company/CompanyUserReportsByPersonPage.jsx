/**
 * Company admin: view reports for a specific company member.
 * Route: /company/user/:userId/reports
 *
 * Uses /admin-reports which is company-scoped for company_admin.
 */
...
const CompanyUserReportsByPersonPage = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { userId } = useParams();

  const displayName = location?.state?.userName || 'User';
  const targetUserId = useMemo(() => normalizeId(userId), [userId]);
  ...
  const fetchReports = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        // Backend may support different param names; try both.
        ...
        // Fallback: if backend ignores user filter, filter in UI.
        const filtered = Array.isArray(serverReports)
          ? serverReports.filter((r) => getReportUserId(r) === targetUserId)
          : [];
    ...
    [activeTab, dateRange.end, dateRange.start, reportType, searchQuery, targetUserId]
  );
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      toast.error(typeof error === 'string' ? error : error?.message || 'Failed to download PDF');
    }
  };
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Company-scoped reports filtered to this person.
            </p>
          </div>
        </div>
...
export default CompanyUserReportsByPersonPage;
