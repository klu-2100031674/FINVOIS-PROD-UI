...
const CompanyAdminSidebar = ({
  ...
  const companyIdForRoutes = useMemo(() => {
    return companyAPI.normalizeCompanyId(
      user?.companyId?._id ||
        user?.companyId?.id ||
        user?.companyId?.$oid ||
        user?.company?._id ||
  ...
  const navItems = useMemo(() => {
    const items = [
      { to: '/company/dashboard', icon: LayoutDashboard, label: 'Company Dashboard' },
      { to: '/company/generate', icon: Zap, label: 'Generate Reports' },
      // { to: '/company/theory-pages', icon: BookOpen, label: 'Theory Pages' },
      { to: '/drafts', icon: FolderOpen, label: 'Drafts' },
      { to: '/company/my-reports', icon: FileStack, label: 'My Reports' },
      { to: '/report-help', icon: Inbox, label: getReportHelpNavLabel(user) },
    ];

    if (companyIdForRoutes) {
      items.push({
        type: 'section',
        key: 'company-nav-section',
        label: 'Your company',
      });
      items.push({
    ...
    items.push({ to: '/company/credits', icon: Gift, label: 'Manage Credits' });
    items.push({ to: '/company/reports', icon: FileText, label: 'Company Reports' });
    items.push({ to: '/company/profile', icon: User, label: 'Profile' });

    return items;
  }, [user, companyIdForRoutes]);
