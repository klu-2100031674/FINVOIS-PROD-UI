 ...
 */
export default function ExecutiveBankSidebar({
  bank,
  basePath,
  sidebarOpen,
  mobileMenuOpen,
  ...
  const roleLabel = formatRoleForDisplay(user?.role, user) || `${bank} Executive`;
  ...
  const navItems = [
    ...
    { to: `${basePath}/drafts`, icon: FileText, label: 'Drafts' },
    { to: `${basePath}/reports`, icon: FileStack, label: 'Reports' },
    { to: `${basePath}/profile`, icon: User, label: 'Profile' },
  ];
        ...
        >
          <div className={`flex items-center ${!sidebarOpen ? '' : 'gap-3'}`}>
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-600 to-purple-900 flex items-center justify-center text-white font-semibold flex-shrink-0">
              {user?.name?.[0]?.toUpperCase() || bank[0]}
            </div>
            {sidebarOpen && (
          ...
          <ul className="space-y-1 px-3">
            {navItems.map((item) => (
              <li key={`${item.to}-${item.label}`}>
                <NavLink to={item.to} className={linkClass} end={item.to === dashPath}>
