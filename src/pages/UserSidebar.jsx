...
import { generateHubLandingPath, theoryPagesHubPath } from '../utils/routePaths';
...
const UserSidebar = ({
  ...
  const generatePath = generateHubLandingPath(user);
  const navItems = [
    { to: dashboardPath, icon: LayoutDashboard, label: 'Dashboard' },
    ...(generatePath !== dashboardPath
      ? [{ to: generatePath, icon: Zap, label: 'Generate report' }]
      : []),
    // { to: theoryPagesHubPath(user), icon: BookOpen, label: 'Theory Pages' },
    ...
    { to: '/reports', icon: FileStack, label: 'My reports' },
