...
import {
  fetchMepmaDprNotificationEmails,
  saveMepmaDprNotificationEmails,
...
const MepmaDprEmailOverlay = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const data = await fetchMepmaDprNotificationEmails();
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const data = await saveMepmaDprNotificationEmails(valid);
      setRows(normalizeRows(data?.emails));
...
export default MepmaDprEmailOverlay;
