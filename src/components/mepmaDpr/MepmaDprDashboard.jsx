...
function ApplicantDetailPanel({ submission }) {
  ...
  const extraEntries = Object.entries(submission?.extraFields || {}).filter(
...
const MepmaDprDashboard = ({
  ...
  const handleToggleServiceAvailed = async (id, nextValue) => {
    setTogglingId(id);
    try {
      await updateMepmaDprServiceAvailed(id, nextValue);
      setSubmissions((prev) =>
        prev.map((item) =>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1400px] mx-auto">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">MEPMA AI DPR Dashboard</h1>
...
export default MepmaDprDashboard;
