...
import {
  fetchDprRequestNotificationEmails,
  saveDprRequestNotificationEmails,
...
const DprRequestEmailOverlay = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const data = await fetchDprRequestNotificationEmails();
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const data = await saveDprRequestNotificationEmails(valid);
      setRows(normalizeRows(data?.emails));
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="DPR Request — Notification Emails"
      size="md"
      footer={
        <div className="flex justify-end gap-2">
          ...
          <button
            ...
            className="px-4 py-2 text-sm font-medium text-white rounded-lg bg-purple-500 hover:bg-purple-600 disabled:opacity-50"
          >
            {saving ? 'Saving...' : 'Save'}
          </button>
        </div>
                ...
                <div className="flex flex-wrap gap-4 pl-0.5">
                  ...
                  >
                    <input
                      ...
                      className="h-4 w-4 rounded border-gray-300 text-purple-500 focus:ring-purple-400 disabled:opacity-50"
                    />
                    Send documents
                  </label>
                </div>
...
export default DprRequestEmailOverlay;
