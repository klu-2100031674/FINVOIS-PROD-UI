 ...
 */
const PmegpSchemeMailForm = ({
  fullName,
  hasPmegpFormPayload,
  linkState,
  pmegpFormPath = PMEGP_GENERATE_PATH,
  pmegpAiChatPath = PMEGP_AI_CHAT_PATH,
  supportSource = 'ui:pmegp-scheme-mail',
}) => {
  ...
  useEffect(() => {
    const resolved = resolveSchemeFormData('pmegpForm', linkState);
    setPmegpForm(resolved);
    if (resolved) {
      saveSchemeFormSession('pmegpForm', resolved);
    }
  }, [linkState]);

  const resolvedLinkState = useMemo(
    () => (pmegpForm ? { ...(linkState || {}), pmegpForm } : linkState),
    [linkState, pmegpForm],
  );

  const displayName =
    (fullName && fullName !== 'Applicant' ? fullName : '') ||
    (pmegpForm?.fullName || '').trim() ||
    'Applicant';
  ...
  const handleSubmit = async (e) => {
    ...
    const formData = resolveSchemeFormData('pmegpForm', resolvedLinkState);
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        {!hasFormPayload && (
          <p className="mt-3 text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
            ...
            <Link to={pmegpFormPath} className="font-semibold underline text-amber-900">
              Back to PMEGP form
            </Link>
          </p>
        )}
...
export default PmegpSchemeMailForm;
