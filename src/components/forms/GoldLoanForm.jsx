...
const GoldLoanForm = ({
  ...
  useEffect(() => {
    if (initialData && isEditMode) {
      console.log('📝 [GoldLoanForm] Loading initial data for edit mode:', initialData);
      setFormData(prev => ({
  ...
  const handleSubmit = useCallback(() => {
    ...
    const submissionData = {
      formData: {
        excelData: excelData,
        additionalData: {
          formType: 'Gold Loan Complete Financial Form',
    ...
    if (onFormDataChange) {
      console.log('💾 [GoldLoanForm] Saving raw formData for future editing:', formData);
