...
export const CLIENT_SCREENING_INBOX_OPTION = {
  id: 'inbox',
...
export const TOPIC_OPTIONS = [
  ...
  { id: 'raw-material', label: 'Need Raw Material Support' },
  { id: 'machinery', label: 'Need Machinary Support' },
  {
    id: 'other',
    label:
...
export const optionNameForMail = (opt, otherDetail, rawMaterialDetail, machineryDetail) => {
  if (opt.id === 'other') {
    if (otherDetail?.trim()) {
  ...
  if (opt.id === 'raw-material') {
    if (rawMaterialDetail?.trim()) {
      return `${opt.label}: ${rawMaterialDetail.trim()}`;
    }
    return opt.label;
  }
  if (opt.id === 'machinery') {
    if (machineryDetail?.trim()) {
      return `${opt.label}: ${machineryDetail.trim()}`;
    }
    return opt.label;
  }
  return opt.label;
